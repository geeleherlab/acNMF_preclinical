...
output_directory = "/home/rchapple/scRNAseq/human/analyses/cNMF/output/benchmark/cNMF_simulated_data/results_replicated/deloc_1.00/Seed_13657/"
run_name = 'cNMF_split0'

countfn = "/home/rchapple/scRNAseq/human/analyses/cNMF/input/benchmark/cNMF_simulated_data/results_replicated/data/Part1_Simulations_K14/deloc_1.00/Seed_13657/cnmf_input_benchmark_split0.edited.h5ad"
...
for x in nmf_job_submission.index:
    factorize_cmd = "bsub -P RC -J deloc_1.00_Seed_13657_split0_factorize -R \"span[hosts=1] rusage[mem=10GB]\" -oo %s -eo %s 'python /home/rchapple/software/cNMF/cnmf_v2.0.py factorize --output-dir %s --name %s --jobstart %d --jobend %d'" % (factorize_dir, factorize_dir, output_directory, run_name, nmf_job_submission['Start'][x], nmf_job_submission['End'][x])
...
wait = "bwait -w 'ended(deloc_1.00_Seed_13657_split0_factorize)'"
os.system(wait)
...
combine_cmd = "bsub -P RC -J deloc_1.00_Seed_13657_split0_combine -R\"rusage[mem=400GB]\" -q large_mem -oo %s -eo %s 'python /home/rchapple/software/cNMF/cnmf_v2.0.py combine --output-dir %s --name %s'" % (combine_dir, combine_dir, output_directory, run_name)
print(combine_cmd)
print('\n')
os.system(combine_cmd)

wait_combine = "bwait -w 'ended(deloc_1.00_Seed_13657_split0_combine)'"
...
for x in chain(range(2,61), range(70,210,10)):                                                       
    selected_K = x                                                              
    print("Selected_K =", selected_K, "\n")                                     
    consensus_cmd = "bsub -P RC -J deloc_1.00_Seed_13657_split0_consensus_outlier -R\"rusage[mem=400GB]\" -q large_mem -oo %s -eo %s 'python /home/rchapple/software/cNMF/cnmf_v2.0.py consensus --output-dir %s --name %s --local-density-threshold %.2f --components %d --show-clustering'" % (consensus_dir, consensus_dir, output_directory, run_name, density_threshold, selected_K)
    print('Consensus command for K=%d:\n%s' % (selected_K, consensus_cmd)) 
    os.system(consensus_cmd)
