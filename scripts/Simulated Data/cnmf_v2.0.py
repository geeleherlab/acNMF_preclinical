...
class cNMF():
    ...
    def get_norm_counts(self, counts, tpm,
        ...
        print(zerocells.sum())
        norm_counts.write('normcounts.h5ad')
        if zerocells.sum()>0:
    ...
    def run_nmf(self,
        ...
        """
        #################
        # Step 1: Figure out what the run parameters and kwargs are 
        # COMPLETE!!!!!    
        # run_params is a pandas dataframe (structure below)
        # >>> run_params
        #      n_components  iter    nmf_seed
        #      0                2     0  2207369836
        #      1                2     1  3959502169
        #      2                2     2  3320718605
        #      3                2     3  2830748055
        #      4                2     4  3738458440
        #      ...            ...   ...         ...
        #      3937           200    49  3517888607
        #      3938           200    50  2314592559
        #      3939           200    51  2552215900
        #      3940           200    52  1420146863
        #      3941           200    53  1342820224
        ###################
        self._initialize_dirs()
        run_params = load_df_from_npz(self.paths['nmf_replicate_parameters'])  #data.files = ['data', 'index', 'columns']   columns = ['n_components', 'iter', 'nmf_seed']
        ...
        ################### 
        # Step 2: Determine the structure of the jobs_for_this_worker object and determine what data is stored in it
        # COMPLETE!!!!
        # jobs_for_this_worker function yields: <generator object worker_filter.<locals>.<genexpr> at 0x7fcd974d67d0>
        # for all practical purposes, this just gives back the index of run_params (starts with 0)
        ###################

        ######################
        # 220210
        # try to edit the run params df to only include the params to run in the loop below
        ##############3
        
        #220210
        run_params_jobsubset = run_params.loc[start:end, :]
        
        #220210
        #jobs_for_this_worker = worker_filter(range(len(run_params)), worker_i, total_workers)
        jobs_for_this_worker = worker_filter(range(len(run_params_jobsubset)), worker_i, total_workers)
        
        for idx in jobs_for_this_worker:
            #220210
            #p = run_params.iloc[idx, :]  
            p = run_params_jobsubset.iloc[idx, :]  
            ...
            #220210
            print('[Worker %d]. Starting task %d (Iteration: %d Rank: %d)' % (worker_i, idx, p['iter'], p['n_components']))
            _nmf_kwargs['random_state'] = p['nmf_seed']
            _nmf_kwargs['n_components'] = p['n_components']
            
            #############
            # Step 3: What are the kwargs at this stage
            ############

            (spectra, usages) = self._nmf(norm_counts.X, _nmf_kwargs)
            
            #################
            # Step 4: How and where are the spectra and usages stored
            # How is the pandas dataframe structured?
            ################
            ...
            #################
            # Step 5: Determine how 'worker_i' and 'total_workers' are used for parallelization (description provided by line 421 above)
            # The word 'parallel' is only found twice in this script (lines 421 and 792)
            ################
    ...
    def consensus(self, k, density_threshold_str='0.5', local_neighborhood_size = 0.30,show_clustering = False,
        ...
        #INSERTED THIS CODE ON 12/03/2021
        #When using H5AD files, there was an error message indicating that the H.type was incompatable (error listed on following line)
        #TypeError: H should have the same dtype as X. Got H.dtype = float64.
        #This line of code was recommended by https://github.com/dylkot/cNMF/issues/8
...
if __name__=="__main__":
    ...
    #220210
    parser.add_argument('--jobstart', type=int, help='[factorize] Used for job submission')
    parser.add_argument('--jobend', type=int, help='[factorize] Used for job submisison')
    ...
    #220210
        ...
        cnmf_obj.run_nmf(worker_i=args.worker_index, total_workers=args.total_workers, start=args.jobstart, end=args.jobend)
