...
sample_ls <- c("NB831", "NB837", "NB839", "NB847", "NB849", "NB853", "NB856", "NB883")
for (sample in sample_ls){
  # load cell list
  gep <- readRDS(paste0("mouse_data/Infercnv_mouse_by_sample_",sample,"/cell_GEP_annot_filtered.RDS"))
  cells <- readRDS(paste0("mouse_data/Infercnv_mouse_by_sample_",sample,"/cellnames.RDS"))
  ...
  CNV_regions <- read.table(paste0("mouse_data/Infercnv_mouse_by_sample_",sample,
                                  "/HMM/HMM_CNV_predictions.HMMi6.hmm_mode-samples.Pnorm_0.3.pred_cnv_regions.dat"),
                           header = T)
  infercnv_obj <- readRDS(paste0("mouse_data/Infercnv_mouse_by_sample_", sample,"/prelim/run.final.infercnv_obj"))
  ...
  # remove chr17
  inferCNV_grouped <- inferCNV_grouped %>% select(!starts_with("chr_17"))
  # reformat col names
  ...
  data_sample <- mouse[,colnames(mouse) %in% c(train_normal_ls,train_cancer_ls, valid_normal_ls,
  ...
  raw_grouped <- raw_grouped %>% select(!starts_with("chr_17"))
  ...
  # binning features----
    ...
    # saveRDS(mylogit, paste0("Models/glm_", sample, "_manual.rda"))
    ...
    # saveRDS(forest, paste0("Models/rf_", sample, "_manual.rda"))
    ...
    # saveRDS(svmfit, paste0("Models/svm_", sample, "_manual.rda"))
  ...
  # InferCNV features----
    ...
    glm_confusion <- confusionMatrix(factor(glm_classes), factor(valid$response), positive='1')
    # saveRDS(mylogit, paste0("Models/glm_", sample, "_infercnv.rda"))

    ## fitting random forest
    forest <- randomForest(factor(response)~., data=train)
    forest_pred <- predict(forest, valid)
    rf_confusion <- confusionMatrix(factor(forest_pred), factor(valid$response))
    # saveRDS(forest, paste0("Models/rf_", sample, "_infercnv.rda"))
    ...
    ypred <- predict(svmfit, valid)
    svm_confusion <- confusionMatrix(factor(ypred), factor(valid$response), positive='1')
    # saveRDS(svmfit, paste0("Models/svm_", sample, "_infercnv.rda"))
  ...
  # write_csv(summ, paste0("Figure/model_acc.csv"), append= TRUE)
  ...
  # write_csv(df, paste0("Figure/GLM_score_", sample, ".csv"))
  ...
  ggsave(paste0("Figure/glm_score_by_feature_", sample, ".svg", width = 3, height = 3, units = "in")
}
